...
def initBlockBitset():
    ...
    block_bitset = [[[False, False, False, 9] for _ in range(9)] for _ in range(9)]
...
# blocks[13]: flag
# blocks[14]: wonder
block_names = [os.path.join("resources", "images", str(x) + ".bmp") for x in range(15)]
blocks = [pygame.image.load(block_name) for block_name in block_names]

number_block_names = [os.path.join("resources", "images", "d" + str(x) + ".bmp") for x in range(10)]
